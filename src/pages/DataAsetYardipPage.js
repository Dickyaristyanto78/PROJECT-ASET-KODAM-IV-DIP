...
const DataAsetYardipPage = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch specifically yarsip-assets endpoint
      const res = await axios.get(`${API_URL}/yarsip-assets`);
      setAssets(res.data || []);
  ...
  const handleDeleteAsset = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          await axios.delete(`${API_URL}/yarsip-assets/${id}`);
  ...
  const handleSaveAsset = async (updatedData) => {
    ...
    try {
      const response = await axios.put(
        `${API_URL}/yarsip-assets/${editingAsset.id}`,
        finalData
      );
      setAssets(
      ...
      {process.env.NODE_ENV === "development" && (
        <Alert variant="success" className="mb-3">
          <small>
            ...
            <br />- API Endpoint: {API_URL}/yarsip-assets
